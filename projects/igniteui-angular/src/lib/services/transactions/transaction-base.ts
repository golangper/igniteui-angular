        ...
        switch (change.type) {
            case ChangeType.ADD:
                if (state) {
                    ...
                    throw new BaseException(`Cannot add this change. Change with id: ${change.id} has been already added.`);
                }
                break;
            case ChangeType.DELETE:
            case ChangeType.UPDATE:
                if (state && state.type === ChangeType.DELETE) {
                    ...
                    throw new BaseException(`Cannot add this change. Change with id: ${change.id} has been already deleted.`);
                }
                if (!state && !originalValue) {
                    //  cannot initially change or delete item with no original value
                    throw new BaseException(`Cannot add this change. This is first change of type ${change.type} for id ${change.id}.
